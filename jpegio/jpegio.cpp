...
{
	if (!dataIsValidJPEG(mem_src, size))printf("JPEG file has some error\n");
...
int jpeg_decode(unsigned char * mem_src, int size, unsigned char * mem_dst, int width, int height)
{
	if (!dataIsValidJPEG(mem_src, size))printf("JPEG file has some error\n");
	struct	jpeg_decompress_struct	cinfo;
	struct	jpeg_error_mgr	jerr;
...
{
	uchar* buffptr = (uchar*)&buff[0];
	int w, h, c;
	getHeader((uchar*)buffptr, (int)buff.size(), w, h, c);

	if (dest.cols != w || dest.rows != h || dest.channels() != c || dest.depth() != CV_8U)
	{
		dest.create(Size(w, h), CV_MAKE_TYPE(CV_8U, c));
	}

	jpeg_decode(buffptr, (int)buff.size(), dest.data, dest.cols, dest.rows);
	return 0;
}
